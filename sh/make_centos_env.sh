...
if [ -e /usr/local/lib/libmsgpack_rpc_client.so.0.0.1 ]; then
    echo "$SCRIPT_NAME msgpack-rpc-c is already installed.."
else
    echo "$SCRIPT_NAME trying to install msgpack-rpc-c.."
    git clone https://github.com/overlast/msgpack-rpc-c.git
    cd msgpack-rpc-c
    waf configure
    waf build
    sudo waf install

    echo "$SCRIPT_NAME making clean msgpack-rpc-c directory.."
    cd $TMP_DIR
    rm -rf msgpack-rpc-c
